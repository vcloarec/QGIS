...
QgsInterpolatedLineRendererWidget::QgsInterpolatedLineRendererWidget( QgsVectorLayer *layer, QgsStyle *style, QgsFeatureRenderer *renderer ) :
  QgsRendererWidget( layer, style )
{
  ...
  connect( mSpinBoxWidth, qOverload< double >( &QDoubleSpinBox::valueChanged ), this, &QgsInterpolatedLineRendererWidget::widgetChanged );
...
QgsFeatureRenderer *QgsInterpolatedLineRendererWidget::renderer()
{
  if ( !mRenderer )
    return nullptr;
  mRenderer->setColorRampShader( mColorRampShaderWidget->shader() );
  ...
  mRenderer->setInterpolatedWidth( interpolatedWidth );
...
void QgsInterpolatedLineRendererWidget::syncToRenderer()
{
  ...
  double minColor = mRenderer->colorRampShader().minimumValue();
  double maxColor = mRenderer->colorRampShader().maximumValue();
  setLineEditValue( mLineEditColorMin, minColor );
  setLineEditValue( mLineEditColorMax, maxColor );
  whileBlocking( mColorRampShaderWidget )->setFromShader( mRenderer->colorRampShader() );
  whileBlocking( mColorRampShaderWidget )->setMinimumMaximum( minColor, maxColor );

  const QgsInterpolatedLineWidth &interpolatedWidth = mRenderer->interpolatedWidth();
