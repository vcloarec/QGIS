...
QgsMeshDatasetGroupTreeItemDelagate::QgsMeshDatasetGroupTreeItemDelagate( QObject *parent )
  ...
  , mExpressionPixmap( QStringLiteral( ":/images/themes/default/mIconExpression.svg" ) )
{
}
...
void QgsMeshDatasetGroupTreeItemDelagate::paint( QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index ) const
{
  ...
  bool isExpression = index.data( QgsMeshDatasetGroupTreeModel::IsExpression ).toBool();
  if ( isVector )
  {
  ...
  if ( isMemory )
    ...
    painter->drawPixmap( iconRect( option.rect, 3 ), mExpressionPixmap );
