...
QgsMeshDatasetGroupTreeItem::QgsMeshDatasetGroupTreeItem( const QDomElement &itemElement, const QgsReadWriteContext &context )
{
  ...
  if ( itemElement.hasAttribute( QStringLiteral( "dataset-group-type" ) ) )
    mDatasetGroupType = static_cast<DatasetGroupType>( itemElement.attribute( QStringLiteral( "dataset-group-type" ) ) .toInt() ) ;

  if ( itemElement.hasAttribute( QStringLiteral( "information" ) ) )
    mInformation = itemElement.attribute( QStringLiteral( "dataset-group-type" ) );
...
QgsMeshDatasetGroupTreeItem::DatasetGroupType QgsMeshDatasetGroupTreeItem::datasetGroupType() const
{
  return mDatasetGroupType;
}

void QgsMeshDatasetGroupTreeItem::setDatasetGroupType( QgsMeshDatasetGroupTreeItem::DatasetGroupType datasetGroupType )
{
...
QString QgsMeshDatasetGroupTreeItem::information() const
{
  return mInformation;
}

void QgsMeshDatasetGroupTreeItem::setInformation( const QString &information )
{
...
QDomElement QgsMeshDatasetGroupTreeItem::writeXml( QDomDocument &doc, const QgsReadWriteContext &context )
{
  ...
  itemElement.setAttribute( QStringLiteral( "information" ), mInformation );
...
bool QgsMeshDatasetGroupTreeItem::isTemporary() const
{
  if ( mDatasetGroupType == File )
    return false;

  if ( mDatasetGroupType == Memory || mDatasetGroupType == None )
    return true;

  //check if dependent on dataset group is temporary
  bool ret = false;
  QgsMeshDatasetGroupTreeItem *root = rootItem();
  for ( int index : mDatasetGroupDependentOn )
  {
...
QString QgsMeshMemoryDatasetGroup::information() const {return QObject::tr( "Memory Dataset Group, do not persit when closing the project" );}
...
QString QgsMeshDatasetGroup::information() const
{
  return QString();
}
